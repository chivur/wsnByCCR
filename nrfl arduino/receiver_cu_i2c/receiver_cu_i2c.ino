...
#define GAS_THRESHHOLD -1 /* TODO define gas threshhold after testing */
...
const uint16_t send_address = 05;/* addr of a node i want to send data to */
...
void loop(){
  ...
  while(network.available()){
    ...
    if(data.name == SENSOR_PRESSURE_NAME){
      Serial.print(" pressure sensor : "); Serial.println(data.val1);
  ...
  if(i2c_data_type == TYPE_CMD){
    ...
    switch(i2c_cmd_idx){ /* first find out what kind of data you have to send */
      case TEMP_IDX:
        ...
        if(i2c_data_idx>4){ /* TODO check if condition is good with '4' -- with '5' it had problems */
      ...
      case HUMID_IDX:
        ...
        break;
      case GAS_IDX:
        if(latest_gas ==0){ /* TODO check what value THRESHHOLD must have */
         /* danger detected */
        ...
        break;
      case MOTION_IDX:
        Wire.write(latest_pir_motion);/* only have to send 1 int */
        if(latest_pir_motion == PIR_MOTION_STOPPED){
          latest_pir_motion=0; /* prevent from sending motion stopped in a loop */  
        }
        i2c_cmd_idx++;
        i2c_data_type = TYPE_CMD;
        break;
